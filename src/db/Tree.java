...
public class Tree {
	...
	private static Node buildDKNode() {
		...
		while (list.size() > 1) {
			java.util.Collections.sort(list, new Comparator<Node>() {//自定义比较方式做排序操作
				@Override
				public int compare(Node n1, Node n2) {
					...
					if (n1.isLeafNode) {
						...
						num1 = ((InternalNode) n1).getNumberTable();
					}
					if (n2.isLeafNode) {
						num2 = 1;
					} else {
						num2 = ((InternalNode) n2).getNumberTable();
					}
					return num1 - num2;
				}
			});
			...
			Node inner = buildInternal(left, right, null);//做笛卡尔积
			list.add(inner);
		}
		return list.get(0);
	}
